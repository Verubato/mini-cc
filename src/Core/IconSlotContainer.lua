...
function M:Layout()
	-- Calculate positions only for used slots
	local usedSlots = {}
	for i = 1, #self.Slots do
		if self.Slots[i].IsUsed then
			table.insert(usedSlots, i)
		end
	end

	local usedCount = #usedSlots
	local totalWidth = (usedCount * self.Size) + ((usedCount - 1) * self.Spacing)
	self.Frame:SetSize(totalWidth > 0 and totalWidth or self.Size, self.Size)
	...
	-- Hide unused slots
	for i = 1, #self.Slots do
		if not self.Slots[i].IsUsed then
			self.Slots[i].Frame:Hide()
		end
	end
end
...
function M:SetLayer(
	...
	local slot = self.Slots[slotIndex]
	if not slot or slotIndex > self.Count then
		return
	end
